...
- **`master`** is at commit `39f23a7` — includes the GitHub Pages deploy (PR #1) and the Vercel base-path fix (PR #3), both merged.
- **Canonical URL is decided: GitHub Pages.** https://srgarner17.github.io/focus-scheduler/ — this is the one to use for the iPad. Vercel (https://focus-scheduler-sand.vercel.app/) is confirmed working too and can stay up as a backup, but isn't the primary link.
...
- **Immediate next action:** physically set up the iPad (see [Next steps](#next-steps) below) — nothing blocking it now that a canonical URL is chosen and both deployments render correctly.
...
- [x] **Live hosting** — deployed to GitHub Pages via GitHub Actions (auto-deploys on every push to `master`): https://srgarner17.github.io/focus-scheduler/ — **this is the canonical URL**
...
- [x] **Vercel** — account created, project imported, and Deployment Protection split (Production public, Preview still gated behind Vercel login). A build bug that broke Vercel specifically (hardcoded GitHub Pages subpath) was found and fixed. Confirmed working at https://focus-scheduler-sand.vercel.app/ — kept as a backup, GitHub Pages is primary

## Open decisions

- ~~Which hosting URL is "the" one?~~ **Resolved:** GitHub Pages is canonical.
...
1. **Set up the first iPad for real** — open https://srgarner17.github.io/focus-scheduler/ in Safari, Add to Home Screen, enable Guided Access (Settings → Accessibility) for wall-mounting, and set a real parent PIN (the `1234` used during testing was only ever test data, never shipped).
2. **Customize the actual schedule** — Morning Routine and Chores items are still the placeholder set from setup; only Soccer Prep's days have been tailored to his real schedule so far.
3. **Scope and build the backend** (Firebase Firestore) once you're ready for multi-device sync and remote visibility — needed before mounting a second iPad anywhere.
4. **(Nice-to-have) custom app icon** — the home-screen icon currently uses the generic default favicon from setup, not a designed icon.
