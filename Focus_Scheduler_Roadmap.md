...
- **`master`** has everything merged through PR #24 (fixed Step 1's mobile-overflow and layout-shift UAT findings). Nothing else outstanding on master.
...
- **Immediate next action:** the editing UX redesign is being delivered as a sequence of small, independently-verifiable steps rather than one big change (see [Incremental delivery plan](#editing-ux-redesign-ready-to-pick-up-later)) — Step 2 (debounced autosave proven on the child's-name field) is done, PR open. Step 3 (extend the same pattern to category name/emoji) is next once that PR merges.
...
1. **Editing UX redesign, Step 3 onward** — see [full scope + incremental delivery plan](#editing-ux-redesign-ready-to-pick-up-later) below. Steps 1-2 (save-status indicator, debounced name field) are done (2026-08-26); Step 3 (extend debounce to category name/emoji) is next.
